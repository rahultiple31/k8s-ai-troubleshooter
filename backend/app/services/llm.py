...
class LLMService:
    ...
    def _fallback_answer(self, question: str, result: dict, sources: list):
        ...
        return (
            ...
            f"### Fix steps\n"
            f"Start with the commands below, then update the workload or cluster resource that matches the failing event.\n\n"
            f"### Commands to verify\n"
            f"{commands or '- `kubectl get events -A --sort-by=.lastTimestamp`'}\n\n"
            f"### Risk / caution\n"
            f"Do not delete pods, PVCs, secrets, or nodes until you confirm the owner workload and data impact.\n\n"
            f"### Sources\n"
            f"{source_lines}"
        )
